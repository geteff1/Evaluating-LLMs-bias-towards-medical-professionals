...
#Use GPT-3.5 to post-process the output from Mistral to format it into json.
def get_completion(prompt, model="gpt-3.5-turbo-1106", retries=3000):
...
def invoke_mistral_with_text(prompt):
    ...
    try:
        ...
        print(f"Couldn't invoke Mistral. Here's why: {err.response['Error']['Code']}: {err.response['Error']['Message']}")
        raise
...
for content_row in range(start_row, len(sheet['F']) + 1):
    ...
    for run in range(1):  # Run each prompt 5 times
        ...
        full_prompt = f"[INST]{prompt} {content}[/INST]"
        
        mistral_response = invoke_mistral_with_text(full_prompt)
